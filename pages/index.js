...
export default function Home() {
  ...
  // 升級版：擬真電路光點跑動設定 (Circuit Electricity Effect)
  const particlesOptions = {
    fpsLimit: 60,
    particles: {
      color: { value: "#D4AF37" }, // 金色光點
      number: { value: 60, density: { enable: true, area: 800 } }, // 增加數量
      opacity: {
        value: 0.6,
        random: { enable: true, minimumValue: 0.1 } // 隨機透明度，模擬閃爍
      },
      shape: { type: "circle" }, // 圓形光點
      size: {
        value: 3,
        random: { enable: true, minimumValue: 1 } // 隨機大小
      },
      move: {
        enable: true,
        speed: { min: 1, max: 6 }, // 隨機速度，有些快有些慢
        direction: "none", // 隨機方向跑動
        random: true, // 隨機路徑
        straight: false, // 不走直線
    ...
    interactivity: {
      events: {
        onHover: { enable: true, mode: "grab" }, // 滑鼠靠近會吸引光點
        onClick: { enable: true, mode: "push" } // 點擊會發射光點
      },
      modes: {
    ...
    detectRetina: true,
    background: {
        color: "transparent" // 保持透明，顯示下方的 PCB 圖片
    }
  };
  ...
  return (
    <div className="bg-[#0A0A0A] min-h-screen text-white selection:bg-[#D4AF37] selection:text-black font-sans relative overflow-hidden">
      ...
      {/* Hero Section - 升級版：擬真 PCB 背景 + 亂跑的電路光點 */}
      <section className="min-h-[80vh] md:min-h-[90vh] flex items-center px-6 md:px-20 relative z-10 overflow-hidden border-b border-white/5 bg-[#0A0A0A]">
        
        {/* 底層：擬真 PCB 紋理背景 (請確保已上傳 public/pcb_bg.png) */}
        <div className="absolute inset-0 z-0 opacity-15 pointer-events-none">
          <img 
            src="/pcb_bg.png" // 如果您還沒上傳，Vercel 會顯示空白，您可以暫時換成臨時網址測試
            // src="https://images.unsplash.com/photo-1591799264318-7e698ddb7c1d?q=80&w=2000" // 臨時網址
            alt="PCB Layout Texture"
        ...
        {/* 背景裝飾光暈 */}
        <div className="absolute top-1/3 left-1/4 w-96 h-96 bg-[#D4AF37]/5 rounded-full blur-[150px] -z-10"></div>
              ...
              <div key={work.id} className="group relative">
                ...
                <div className="relative aspect-video overflow-hidden bg-[#1A1A1A] mb-5 shadow-[0_0_30px_rgba(212,175,55,0.1)] group-hover:shadow-[0_0_60px_rgba(212,175,55,0.3)] transition-all duration-700">
                  <iframe
                    ...
                    src={`https://www.youtube.com/embed/${work.video_id}?rel=0&showinfo=0&autoplay=0&mute=1`}
