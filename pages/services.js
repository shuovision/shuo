    ...
    <div className="bg-[#0F0F0F] min-h-screen text-white font-sans selection:bg-[#D4AF37] selection:text-black">
      ...
      <main className="max-w-6xl mx-auto px-8 py-20 md:py-32">
        <section className="mb-20">
          <h2 className="text-[#D4AF37] text-xs tracking-[0.5em] mb-4 uppercase opacity-60 font-mono">Technical Expertise</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-16">
          {serviceList.map((service) => (
            <div key={service.id} className="border-t border-white/10 pt-8 group hover:border-[#D4AF37]/30 transition-colors">
              <div className="flex justify-between items-start mb-6">
                <div>
                  ...
                  <p className="text-[#D4AF37] text-xs font-medium tracking-widest font-mono">{service.subtitle}</p>
                </div>
              </div>
              <p className="text-gray-400 text-sm leading-loose mb-8 md:h-20">
                {service.desc}
              </p>
              <div className="flex flex-wrap gap-2">
                {service.tags.map(tag => (
                  <span key={tag} className="text-[9px] border border-white/20 px-3 py-1 rounded-full text-gray-500 uppercase tracking-tighter group-hover:border-[#D4AF37]/50 group-hover:text-[#D4AF37] transition-all font-mono">
                    {tag}
                  </span>
                ))}
              </div>
        ...
        {/* 底部聯繫 */}
        <section className="mt-40 bg-[#1A1A1A] p-10 md:p-20 text-center rounded-sm border border-white/5 shadow-2xl relative overflow-hidden">
          ...
          <Link href="/contact">
            <a className="bg-[#D4AF37] text-black px-12 py-4 rounded-full font-bold tracking-widest hover:bg-white transition-all shadow-lg inline-block">
              開始諮詢
            </a>
          </Link>
        </section>
      ...
      <footer className="py-16 text-center opacity-20 text-[9px] tracking-[0.3em] uppercase border-t border-white/5">
